    ...
    <div class="grid-col1"> 
      <div class="apptGrid">
        <?php 
          ...
          //Sets initial error notifications
          $fnameErr = $lnameErr = $phoneErr = $addressErr = $apptTimeErr = $apptDateErr = $dentistErr = "";

          //Sets initial customer information
          $fname = $lname = $phone = $address = $apptTime = $apptDate = $dentist = "";

          /* Each of these goes through the user's inputs and makes sure they aren't empty. 
          If it is empty, an error message is added and the initial customer information isn't set. */
          if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (empty($_POST["time"])) {
                $apptTimeErr = "Time is required";
            } else {
                $apptTime = ($_POST["time"]);
            }

            if (empty($_POST["date"])) {
              $apptDateErr = "Date is required";
           } else {
               $apptDate = ($_POST["date"]);
           }

            if (empty($_POST["dentist"])) {
                $dentistErr = "Dentist is required";
            } else {
                $dentist = ($_POST["dentist"]);
            }
          }

          /*Goes through the user's inputs, determines if any are empty, 
          and prints the applicable error messages if any are.
          Nothing is added to the database in this case.*/
